...
// 디자인 토큰 (사업주용 → 신뢰성과 전문성)
const B = {
  bg: "#FAF7F2", card: "#FFFEF9", ink: "#1C1917", ink2: "#292524",
  amber: "#D97706", amberP: "#FEF3C7", navy: "#1B3A6B", navyP: "#EFF4FB",
  g50: "#F9F8F6", g100: "#EDEAE5", g300: "#C8C4BB", g500: "#8C8880", g700: "#4A4640",
  red: "#DC2626", redP: "#FEF2F2", green: "#16A34A", greenP: "#F0FDF4",
};

// 디자인 토큰 (근로자용 → 따뜻하고 접근하기 쉬운 느낌)
const W2 = {
  bg: "#F0FAF7", card: "#FFFFFF", ink: "#1A2E2A", ink2: "#2D4A44",
  teal: "#0D9488", tealP: "#CCFBF1", coral: "#E85D3A", coralP: "#FEF0EC",
  g50: "#F7FBFA", g100: "#E0F0ED", g300: "#9ECDC5", g500: "#5A9E96", g700: "#2D6B62",
  red: "#DC2626", redP: "#FEF2F2", green: "#059669", greenP: "#ECFDF5",
  yellow: "#D97706", yellowP: "#FFFBEB",
};
...
const RoleLanding = ({ onSelect, waitCount }) => (
  <div style={{ minHeight: "100vh", background: "#0F172A", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
    <div className="fu" style={{ textAlign: "center", marginBottom: 52 }}>
      <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 52, fontWeight: 900, color: "#F8FAFC", letterSpacing: "-.02em", marginBottom: 8 }}>
        NOR<span style={{ color: "#D97706" }}>MU</span>
      </div>
      <p style={{ color: "#94A3B8", fontSize: 16, lineHeight: 1.75 }}>
        근로감독관이 오기 전에 → 먼저 알고, 먼저 지켜내는 노무 플랫폼<br />
        <span style={{ color: "#64748B", fontSize: 13 }}>고용노동부 마이크로데이터 300만 레코드 기반</span>
      </p>
    </div>
    ...
    <div className="fu1" style={{ display: "flex", gap: 18, flexWrap: "wrap", justifyContent: "center", maxWidth: 680, width: "100%" }}>
      <button onClick={() => onSelect("boss")}
        style={{ flex: "1 1 280px", background: "linear-gradient(135deg,#1B3A6B 0%,#1e3a5f 100%)", borderRadius: 20, padding: "36px 28px", textAlign: "left", border: "2px solid rgba(255,255,255,.08)", position: "relative", overflow: "hidden", transition: "all .2s" }}
        onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = "0 20px 48px rgba(0,0,0,.4)" }}
        onMouseLeave={e => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = "" }}>
        <div style={{ fontSize: 38, marginBottom: 14 }}>🏢</div>
        <div style={{ fontSize: 22, fontWeight: 800, color: "#F8FAFC", marginBottom: 8 }}>사업주님들</div>
        <div style={{ fontSize: 14, color: "#93C5FD", lineHeight: 1.65, marginBottom: 18 }}>
          근로감독 리스크 사전 감지<br />업종별 위험 특화된 TOP3<br />계약서 자동생성 툴
        </div>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(217,119,6,.3)", borderRadius: 99, padding: "6px 14px", fontSize: 12, fontWeight: 700, color: "#FCD34D" }}>지금 시작하기 →</span>
      </button>

      <button onClick={() => onSelect("worker")}
        style={{ flex: "1 1 280px", background: "linear-gradient(135deg,#065F46 0%,#047857 100%)", borderRadius: 20, padding: "36px 28px", textAlign: "left", border: "2px solid rgba(255,255,255,.08)", position: "relative", overflow: "hidden", transition: "all .2s" }}
        onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = "0 20px 48px rgba(0,0,0,.4)" }}
        onMouseLeave={e => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = "" }}>
        <div style={{ fontSize: 38, marginBottom: 14 }}>👤</div>
        <div style={{ fontSize: 22, fontWeight: 800, color: "#F8FAFC", marginBottom: 8 }}>근로자·알바생님들</div>
        <div style={{ fontSize: 14, color: "#6EE7B7", lineHeight: 1.65, marginBottom: 18 }}>
          내 알바 최저임금 맞나요?<br />초과근무·휴일·야간 확인<br />4대보험 체크 + 신고 안내
        </div>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(5,150,105,.3)", borderRadius: 99, padding: "6px 14px", fontSize: 12, fontWeight: 700, color: "#A7F3D0" }}>무료 확인 →</span>
      </button>
    </div>

    <div className="fu2" style={{ marginTop: 32, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,.05)", borderRadius: 99, padding: "8px 18px" }}>
        <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#4ADE80", animation: "pulse 2s infinite" }} />
        <span style={{ color: "#94A3B8", fontSize: 13 }}>현재 <strong style={{ color: "#F8FAFC" }}>{waitCount}명</strong> 이용 중</span>
      </div>
      <div style={{ display: "flex", gap: 20, color: "#475569", fontSize: 12 }}>
...
const BossApp = ({ onBack }) => {
  ...
  const runAI = async () => {
    setAL(true); setAT(""); saveAction('ai_analysis_start', { industry: industry2 });
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST", 
        headers: { 
          "Content-Type": "application/json",
          "x-api-key": import.meta.env.VITE_ANTHROPIC_API_KEY || "sk-ant-api03-demo",
          "anthropic-version": "2023-06-01"
        },
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514", max_tokens: 800,
          system: `당신은 대한민국 근로감독관입니다. 고용부 마이크로데이터(vw_api_yearly_industry_size, 업종코드: ${iData.code})를 기반으로 실무 조언하세요. ${industry2} 3년 실데이터: 고용보험가입률 ${iData.ins}%, 퇴직급여설정률 ${iData.ret}%, 초과근로시간 ${iData.ot}h/월. 200자 이내, 구체적 수치 포함하여 핵심 리스크만 조언.`,
          messages: [{ role: "user", content: `${industry2}(DB코드: ${iData.code}) 사업장의 핵심 근로감독 리스크를 알려주세요.` }]
        })
      });
      const d = await res.json(); setAT(d.content?.[0]?.text || "분석 실패");
    } catch (e) { setAT(`오류: ${e.message}`); }
    finally { setAL(false); }
  };
...
const WorkerApp = ({ onBack }) => {
  ...
  const TABS = [{ id: "check", label: "내 임금 확인", icon: "💰" }, { id: "rights", label: "내 권리", icon: "✅" }, { id: "report", label: "신고 방법", icon: "📢" }];
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "20px 20px 80px" }}>
        ...
        {tab === "report" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {[
              { icon: "📞", title: "고용노동부 콜센터", sub: "☎️ 1350 (24시간 운영)", desc: "전화 상담으로 신고 가능.", link: "tel:1350", btn: "1350 전화" },
              { icon: "🌐", title: "고용노동부 민원포털", sub: "minwon.moel.go.kr", desc: "온라인 신고 제공. 익명 신고 가능.", link: "https://minwon.moel.go.kr", btn: "온라인 신고" },
              { icon: "💬", title: "체불진단원 상담센터", sub: "☎️ 1644-3119", desc: "체불(미지급 임금) 전문 상담.", link: "tel:16443119", btn: "체불 전화" },
              { icon: "🔒", title: "NORMU 익명 제보", sub: "개인정보 전혀 수집안함", desc: "지금 신고가 어려우시면 지역구 위험도와 신고 방법을 알려드립니다.", link: "#", btn: "익명 제보" },
            ].map((r, i) => (
            ...
            <div style={{ background: c.yellowP, borderRadius: 12, padding: "16px 18px", border: `1px solid rgba(217,119,6,.2)` }}>
              ...
              {["근로계약서 사본 (없으면 콜라카콜·문자 캡쳐)", "기업카드 현금영수증 발급 기록 내역", "근로 증거 (다음·오픈톡 OK)", "체납 일지표"].map((v, i) => (
