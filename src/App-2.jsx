    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
      ...
      <div className="fu1" style={{ display: "flex", gap: 18, flexWrap: "wrap", justifyContent: "center", maxWidth: 680, width: "100%" }}>
        ...
        <button onClick={() => onSelect("worker")}
          ...
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(16,185,129,.3)", borderRadius: 99, padding: "6px 14px", fontSize: 12, fontWeight: 700, color: "#6EE7B7" }}>내 권리 확인하기 ✅</span>
        </button>
      </div>
            ...
            <div style={{ background: c.card, borderRadius: 13, padding: "20px 22px", border: `1px solid ${c.g100}` }}>
              ...
              <button 
                onClick={async () => {
                  ...
                  try {
                    const wage = contractData.workType === "단시간(파트타임)"
                      ? `시급 ${Number(contractData.hourlyWage).toLocaleString()}원`
                      : `월급 ${Number(contractData.monthlyWage).toLocaleString()}원`;
                    const res = await fetch("https://api.anthropic.com/v1/messages", {
                      ...
                      body: JSON.stringify({
                        ...
                        messages: [{
                          role: "user",
                          content: `당신은 대한민국 노무사입니다. 아래 정보를 바탕으로 2026년 근로기준법을 완벽히 준수하는 표준 근로계약서를 작성해주세요.

직무: ${contractData.jobTitle}
근로형태: ${contractData.workType}
주 근로시간: ${contractData.workHours}시간
임금: ${wage} (2026년 최저시급 10,500원 이상 준수)
근무시작일: ${contractData.startDate}
근무장소: ${contractData.workPlace || "사업장 내"}
...
- 근무장소 및 업무내용
- 근로시간, 휴게시간
- 임금 구성 (기본급, 주휴수당, 각종 수당)
- 연차유급휴가
- 퇴직금 (해당 시)
- 4대보험

실제 계약서처럼 완성된 형태로 작성해주세요. 빈칸(_____)으로 된 항목은 실제 내용으로 채워주세요.`
                        }]
                      })
                    });
                    const data = await res.json();
                    setGeneratedContract(data.content?.[0]?.text || "생성 실패");
                  } catch(e) {
                    setGeneratedContract(`오류: ${e.message}`);
                  } finally {
              ...
              >
                {generatingContract ? "⏳ AI가 계약서를 작성 중..." : "📝 근로계약서 생성하기"}
              </button>
            </div>

            {/* 생성된 계약서 */}
            {generatedContract && (
              <div style={{ background: c.card, borderRadius: 13, border: `1px solid ${c.g100}`, overflow: "hidden" }}>
                <div style={{ background: c.navyP, padding: "12px 18px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: c.navy }}>✅ 근로계약서 생성 완료</div>
                  <button
                    onClick={() => { navigator.clipboard.writeText(generatedContract); alert("클립보드에 복사됐습니다!"); }}
                    style={{ padding: "6px 14px", background: c.navy, color: "#fff", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer" }}
                  >📋 복사</button>
                </div>
                <pre style={{ margin: 0, padding: "18px 20px", fontSize: 12, color: c.ink, lineHeight: 1.8, whiteSpace: "pre-wrap", fontFamily: "inherit", maxHeight: 420, overflowY: "auto" }}>
                  {generatedContract}
                </pre>
                <div style={{ padding: "12px 18px", background: c.g50, borderTop: `1px solid ${c.g100}` }}>
                  <div style={{ fontSize: 11, color: c.g500 }}>
                    💡 복사 후 워드/한글에 붙여넣기 하거나, 출력 후 서명란을 추가하세요. 사업주·근로자 각 1부 보관.
                  </div>
                </div>
              </div>
            )}
                    ...
                    try {
                      ...
                      const msgContent = isImage
                        ? [
                            ...
                            { type: "text", text: `당신은 대한민국 노무사입니다. 위 이미지는 근로계약서입니다. 2026년 현행 근로기준법 기준으로 문제점을 분석해주세요.
  ...
  "problems": [
    {
      ...
      "title": "문제 제목",
      "desc": "구체적인 문제 설명 (수치 포함)",
      "law": "관련 법조항"
    }
  ],
  "good_points": ["잘 된 점 1", "잘 된 점 2"]
}

type은 반드시 지켜야 할 위반 시 "error", 권고사항은 "warning"으로 구분하세요.
계약서가 보이지 않거나 판독 불가하면 problems에 {"type":"error","title":"판독 불가","desc":"계약서가 선명하지 않아 분석이 어렵습니다. 더 선명한 사진을 올려주세요.","law":""} 를 반환하세요.` }
                          ]
                        : [{ type: "text", text: `당신은 대한민국 노무사입니다. 아래는 근로계약서 PDF 파일(base64)입니다. 파일명: ${uploadedFile.name}\n\n2026년 현행 근로기준법 기준으로 문제점을 분석해주세요. 파일을 직접 읽을 수 없으므로 파일명과 컨텍스트를 바탕으로 일반적인 계약서 점검 항목을 안내해주세요.\n\n다음 JSON 형식으로만 응답하세요:\n{"summary":"한 줄 요약","problems":[{"type":"warning","title":"PDF 직접 분석 불가","desc":"PDF 파일은 이미지로 변환 후 업로드하시면 정확한 분석이 가능합니다. 아래는 일반적인 체크 항목입니다.","law":""}],"good_points":["파일을 업로드해 주셔서 감사합니다"]}` }];
                      ...
                      setAnalysisResult({
                        ...
                        problems: [{ type: "error", title: "분석 실패", desc: `오류: ${e.message}. 이미지가 너무 크거나 형식이 지원되지 않을 수 있습니다.`, law: "" }],
                        good_points: []
                      });
                    } finally {
                      setAnalyzing(false);
                ...
                >
                  {analyzing ? "🔍 AI가 계약서를 읽는 중..." : "🔍 계약서 분석하기"}
                </button>
              )}
            </div>
            ...
            {analysisResult && (
              <>
                {/* 요약 배너 */}
                <div className="pi" style={{ background: c.redP, borderRadius: 13, padding: "16px 20px", border: `2px solid ${c.red}` }}>
                  <div style={{ fontSize: 14, fontWeight: 800, color: c.red, marginBottom: 4 }}>
                    ⚠️ {(analysisResult.problems||[]).filter(p=>p.type==="error").length}개 치명적 문제 &nbsp;·&nbsp; {(analysisResult.problems||[]).filter(p=>p.type==="warning").length}개 경고
                  </div>
                  {analysisResult.summary && (
                    <div style={{ fontSize: 12, color: "#7F1D1D" }}>{analysisResult.summary}</div>
                  )}
                </div>

                {/* 문제 목록 */}
                {(analysisResult.problems||[]).map((problem, i) => (
                  <div key={i} className="pi" style={{
                    background: c.card, borderRadius: 13, padding: "16px 18px",
                    border: `2px solid ${problem.type === "error" ? c.red : c.yellow}`,
                  }}>
                    <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                      <div style={{ fontSize: 22, flexShrink: 0 }}>{problem.type === "error" ? "🔴" : "⚠️"}</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: problem.type === "error" ? c.red : c.yellow, marginBottom: 4 }}>
                          {problem.title}
                        </div>
                        <div style={{ fontSize: 12, color: c.ink, lineHeight: 1.7, marginBottom: problem.law ? 8 : 0 }}>
                          {problem.desc}
                        </div>
                        {problem.law && (
                          <span style={{ display: "inline-block", background: problem.type === "error" ? c.redP : c.yellowP, color: problem.type === "error" ? c.red : c.yellow, fontSize: 11, fontWeight: 600, padding: "3px 8px", borderRadius: 6 }}>
                            {problem.law}
                          </span>
                        )}
                      </div>
                ...
                {/* 잘된 점 */}
                {(analysisResult.good_points||[]).length > 0 && (
                  <div style={{ background: c.greenP, borderRadius: 13, padding: "16px 18px", border: `1px solid rgba(16,185,129,.25)` }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: c.green, marginBottom: 8 }}>✅ 잘 된 점</div>
                    {(analysisResult.good_points||[]).map((g,i) => (
                      <div key={i} style={{ fontSize: 12, color: c.ink, lineHeight: 1.7 }}>· {g}</div>
                    ))}
                  </div>
                )}

                {/* 다음 단계 */}
                <div style={{ background: c.tealP, borderRadius: 12, padding: "14px 18px", border: `1px solid rgba(20,184,166,.2)` }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: c.teal, marginBottom: 6 }}>💡 다음 단계</div>
                  <div style={{ fontSize: 12, color: c.ink, lineHeight: 1.8 }}>
                    1. 사업주에게 계약서 수정 요청<br/>
                    2. 수정 거부 시 노동청 ☎️ 1350 신고<br/>
                    3. 신고 시 이 화면을 캡처하여 증거로 제출
                  </div>
                </div>

                {/* 다시 분석 */}
                <button onClick={() => { setAnalysisResult(null); setUploadedFile(null); }}
                  style={{ width: "100%", padding: "12px", background: "transparent", color: c.teal, border: `1px solid ${c.teal}`, borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                  ↩ 다른 계약서 분석하기
                </button>
              </>
            )}
